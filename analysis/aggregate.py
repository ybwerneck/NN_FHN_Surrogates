...
def process_and_combine_from_dict(input_dict, output_csv_path):
    ...
    for problem_name, subdir in input_dict.items():
        try:
            ...
            columns = ["Problem", "Folder", "Final Error 1", "Final Error 2", "Hidden Layers", "Pinn Info", "Batch Size","Inf speed"]
            results_table = pd.DataFrame(columns=columns)
            ...
            for folder in os.listdir(subdir):
                ...
                try:
                    ...
                    results_table.loc[len(results_table)] = [
                        problem_name, folder, final_value1, final_value2, hidden_layers, pinn, bs,avg_time
                    ]
                except:
                     print("")
...
data=df
problem_b_inf_speed = data[data['Problem'] == 'B'][['Folder', 'Inf speed']]

# Merge 'Inf speed' from Problem B into the original data, matching by 'Folder'
data = data.merge(problem_b_inf_speed, on='Folder', suffixes=('', '_from_B'), how='left')

# Replace 'Inf speed' in Problem A (or others) with the value from Problem B where applicable
data['Inf speed'] = data['Inf speed_from_B'].combine_first(data['Inf speed'])

# Drop the temporary column created during the merge
data.drop(columns=['Inf speed_from_B'], inplace=True)
...
# Add specific points for problems B and C (e.g., point P2)
point_p2_b = {'Inf speed': 0.62, 'Final Error 1': 0.0032}
point_p2_c = {'Inf speed': 0.72, 'Final Error 1': 0.0042}

# Highlight the points with circles
plt.scatter(point_p2_b['Inf speed'], point_p2_b['Final Error 1'], facecolors='none', edgecolors='green', s=200, linewidth=2)
plt.scatter(point_p2_c['Inf speed'], point_p2_c['Final Error 1'], facecolors='none', edgecolors='blue', s=200, linewidth=2)

# Annotate the points
plt.text(point_p2_b['Inf speed'] * 1.05, point_p2_b['Final Error 1'], 'P2 (B)', fontsize=14, color='green')
plt.text(point_p2_c['Inf speed'] * 1.05, point_p2_c['Final Error 1'], 'P2 (C)', fontsize=14, color='blue')

for problem, group in data.groupby('Problem'):
    print(problem)
    plt.scatter(group['Inf speed'], group['Final Error 1'], label=f'Problem {problem}', 
                color=problem_colors.get(problem, 'gray'), alpha=0.7, edgecolors='k',s=40)
...
plt.savefig('inf_speed_vs_accuracy.pdf', dpi=300)
